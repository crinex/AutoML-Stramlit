...
st.set_page_config(page_title='Auto ML APP')

st.write('# AutoML Test App')
...
def build_model(df):
    ...
    with st.markdown('**R-squared**'):
        ...
        ax1 = sns.barplot(y=predictions_test.index,
                          x="R-Squared", data=predictions_test)
        ax1.set(xlim=(0, 1))
    ...
    ax1 = sns.barplot(x=predictions_test.index,
                      y="R-Squared", data=predictions_test)
    ax1.set(ylim=(0, 1))
    ...
    with st.markdown('**Calculation time**'):
        ...
        ax3 = sns.barplot(y=predictions_test.index,
                          x="Time Taken", data=predictions_test)
    ...
    ax3 = sns.barplot(x=predictions_test.index,
                      y="Time Taken", data=predictions_test)
    plt.xticks(rotation=90)
    st.pyplot(plt)
...
with st.sidebar.header('1. Upload your CSV File'):
    ...
    st.sidebar.markdown("""
    [Example CSV input file](https://raw.githubusercontent.com/crinex/AutoML-Streamlit/master/country_vaccinations.csv)
    """)
